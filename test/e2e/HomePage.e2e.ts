...
const clickToCounterLink = (t: TestController) => t.click(Selector('a').withExactText('to Counter'));
...
test('e2e', async (t) => {
  await t.expect(getPageTitle()).eql('Photos');
});
...
fixture`Counter Tests`.page('../../app/app.html').beforeEach(clickToCounterLink).afterEach(assertNoConsoleErrors);
...
test('should back to home if back button clicked', async (t) => {
  await t.click('[data-tid="backButton"] > a').expect(Selector('[data-tid="container"]').visible).ok();
});
